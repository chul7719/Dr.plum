...
| DB | Prisma + SQLite(개발) → PostgreSQL(운영) | 스키마를 코드로 관리하고, `provider`만 바꾸면 운영 DB로 그대로 이전됩니다. SQLite는 enum을 지원하지 않아 role/status 필드는 String + 주석으로 허용값을 문서화했습니다 (PostgreSQL 전환 시 실제 enum으로 되돌릴 수 있습니다). |
...
cp .env.example .env              # NEXTAUTH_SECRET에 임의의 문자열 채워넣기
npx prisma migrate dev --name init   # SQLite에 테이블 생성
npm run seed                      # 테스트 계정 + 샘플 요청 생성
npm run dev                       # http://localhost:3000
```

`NEXTAUTH_SECRET`은 터미널에서 `openssl rand -base64 32`로 생성한 값을 넣으면 됩니다.
...
| 6. 배포 | ⬜ 미착수 | Vercel/Neon 등 외부 계정이 필요해 이번 작업 범위에서 제외 |

## 참고: 운영 DB(PostgreSQL)로 전환하는 법
...
PostgreSQL은 enum을 지원하므로, 전환하는 김에 `role`/`status` 필드들을 다시
`enum Role { ... }` 형태로 되돌리면 DB 레벨에서도 허용값이 제한됩니다 (지금은
SQLite 제약으로 String + 주석 문서화로 대신하고 있습니다).
