...
webapp/
├── prisma/
│   ├── schema.prisma      # 데이터 모델 (Organization, Store, User, Vendor, EquipmentType, Notice, Request, Quote, CompletionPhoto, Settlement 등)
│   └── seed.ts            # 테스트 계정 + 프랜차이즈 본사 2곳(격리 검증용) + 설비 종류/공지 기본값 + 샘플 요청 생성
├── src/
...
│   │   ├── vendor-scope.ts       # [기능] 본사 전용 업체(vendorId) → organizationId 조회
│   │   └── org-scope.ts          # [기능] role별 "내 본사가 어디인지" 계산 (설비 종류·공지사항 공용)
│   ├── types/next-auth.d.ts # 세션에 role/storeId/vendorId/organizationId 타입 추가
│   ├── components/
│   │   ├── MobileHeader.tsx
│   │   ├── LoginForm.tsx
│   │   ├── SignupForm.tsx   # 회원가입 (역할 선택 + 역할별 소속 입력)
│   │   ├── NoticeList.tsx   # 본사 공지 읽기 전용 목록 (매장·기사 공용)
│   │   ├── mart/            # 매장 담당자 화면 컴포넌트
│   │   ├── technician/      # 협력업체 기사 화면 컴포넌트 (입찰·출발/도착·완료보고+사진·공지)
│   │   └── hq/               # 본사 대시보드·정산·업체·설비 종류·공지사항 컴포넌트
│   └── app/
│       ├── login/
│       ├── signup/                    # 회원가입 페이지
...
│       │   ├── settlements/           # 정산 관리
│       │   ├── vendors/               # 업체 관리 (CRUD)
│       │   ├── equipment-types/       # 설비 종류 관리 (CRUD)
│       │   └── notices/               # 공지사항 관리 (CRUD)
│       └── api/
│           ├── auth/[...nextauth]/, auth/signup/, auth/signup-options/
│           ├── requests/, requests/[id]/, requests/[id]/quotes/  # 입찰 제출 API
│           ├── vendors/, vendors/[id]/
│           ├── equipment-types/, equipment-types/[id]/
│           ├── notices/, notices/[id]/
│           └── settlements/
```
...
테스트 계정 외에 직접 새 계정을 만들 수도 있습니다. 역할을 고르면 그 역할에
필요한 소속 입력이 아래에 나타납니다.
- **매장 담당자**: 기존 본사 소속의 지점을 선택하거나, 새 지점 이름을 입력해 생성
- **협력업체 기사**: 기존 업체를 선택하거나, 새 업체 이름을 입력해 생성
- **본사 관리자**: 항상 새 프랜차이즈 본사를 만들면서 그 본사의 첫 관리자가 됨
  (기존 본사에 다른 관리자를 자유롭게 추가할 수 있게 하면 다른 프랜차이즈의
  정산 데이터까지 노출될 위험이 있어, 지금은 새 본사 생성만 허용합니다)

지금은 "기존 지점/업체 선택"에 초대 코드나 승인 절차가 없어 이메일만 있으면
소속을 자유롭게 고를 수 있습니다. 실서비스로 갈 때는 초대 코드나 본사 승인
절차를 추가하는 게 좋습니다 (`src/app/api/auth/signup/route.ts` 상단 주석 참고).
...
## 본사 관리 기능 (업체 · 설비 종류 · 공지사항)

본사 대시보드(`/hq`) 상단 메뉴에서 3가지를 관리할 수 있습니다. 전부
등록·수정·삭제(CRUD)가 가능하고, 전부 로그인한 본사 관리자의
Organization으로 범위가 제한됩니다 (다른 프랜차이즈 데이터는 안 보임).

- **업체 관리** (`/hq/vendors`, `src/app/api/vendors`) — 본사가 직접
  등록하는 "본사 전용 협력업체"입니다. 기존에 기사가 회원가입 시 만드는
  업체는 여러 본사가 공유하는 마켓플레이스 업체(organizationId 없음)로
  그대로 남아있고, 여기서 등록한 업체만 `organizationId`가 붙어 **그 본사
  소속 매장의 요청에만** 입찰할 수 있습니다 (`src/lib/vendor-scope.ts`가
  입찰 목록 조회·제안 등록 양쪽에서 이 범위를 강제합니다). 소속 기사
  계정이나 제안 이력이 있는 업체는 삭제할 수 없습니다.
- **설비 종류 관리** (`/hq/equipment-types`, `src/app/api/equipment-types`) —
  매장 담당자의 "새 작업요청" 화면 드롭다운에 나오는 설비 종류 목록입니다.
  예전엔 냉장 쇼케이스/냉동고/공조 설비/POS 단말 4개가 코드에 하드코딩돼
  있었는데, 이제 본사가 직접 추가·수정·삭제합니다. 아직 하나도 등록 안 한
  본사는 매장 쪽 화면이 자동으로 직접 입력 텍스트박스로 대체됩니다.
- **공지사항** (`/hq/notices`, `src/app/api/notices`) — 본사가 작성하면
  그 본사 소속 매장 담당자 앱(`/mart/notices`)과 기사 앱("공지" 탭)에
  똑같이 노출됩니다. 공용 마켓플레이스 업체 소속 기사(특정 본사에 속하지
  않음)에게는 공지가 보이지 않습니다.

세 기능 모두 role별로 "내가 어느 본사 소속인지"를 계산해야 하는데, 그
계산 방식이 role마다 달라서(HQ_ADMIN은 세션에 바로 있음, MART_MANAGER는
Store를 거쳐서, TECHNICIAN은 Vendor를 거쳐서) `src/lib/org-scope.ts`의
`resolveOrganizationId()` 하나로 통일했습니다.
